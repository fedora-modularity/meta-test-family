...
class CommonFunctions(object):
    ...
    def loadconfig(self):
        ...
        doc_name = ['modularity-testing', 'meta-test-family']
        if self.config.get('document') not in doc_name:
            raise ConfigExc("bad yaml file, not contain any of document: item (%s)" % doc_name)
    ...
    def getPackageList(self, profile=None):
        ...
        if not profile:
            if 'packages' in self.config:
                ...
                packages_profiles = []
                for x in self.config.get('packages',{}).get('profiles',[]):
