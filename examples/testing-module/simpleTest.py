...
class simpleTests(module_framework.AvocadoTest):
    ...
    def testPath(self):
        ...
        if "/usr/share/moduleframework/" in os.path.dirname(__file__):
            self.assertIn("/usr/share/moduleframework/", os.path.dirname(__file__))
            self.assertIn("/usr/lib/python2.7/site-packages/moduleframework", os.path.dirname(module_framework.__file__))
        else:
