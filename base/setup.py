...
class Module(moduleframework.CommonFunctions):
    def __init__(self):
        ...
        self.baseruntimeyaml = self.getModulemdYamlconfig("https://raw.githubusercontent.com/fedora-modularity/check_modulemd/develop/examples-modulemd/base-runtime.yaml")

    def CreateLocalRepo(self):
        allmodulerpms = " ".join(self.whattoinstall['rpms'])
        allbasertrpms = " ".join(self.baseruntimeyaml['data']['profiles']['default']['rpms'])
        utils.process.run("yumdownloader --destdir=%s --resolve %s %s" % (self.rpmsrepo, allmodulerpms, allbasertrpms))
        utils.process.run("cd %s; createrepo --database %s" % (self.rpmsrepo, self.rpmsrepo), shell = True)
        print "file://%s" % self.rpmsrepo

    def CreateContainer(self):
        localfiles = glob.glob('%s/*.rpm' % self.rpmsrepo)
        utils.process.run("dnf -y install --disablerepo=* --allowerasing --installroot=%s %s" % (self.rpmsinstalled, " ".join(localfiles)))
        print "file://%s" % self.rpmsrepo
